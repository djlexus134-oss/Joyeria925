...
try {
    ...
    $catalogoError = true;
    error_log('Catalogo visitante: ' . $e->getMessage());
}
...
/**
 * Precio de referencia (misma lógica que admin/views/pieza/index.php).
 */
function joyeria_precio_referencia_visita(array $pieza): float
{
...
/**
 * Intercala piezas por familia (round-robin) para evitar bloques de la misma familia seguidos.
 *
 * @param array<int, array{nom_familia: string, items: array<int, mixed>}> $piezasPorFamilia
 * @return array<int, mixed>
 */
function joyeria_catalogo_intercalado(array $piezasPorFamilia): array
{
    $queues = [];
    foreach ($piezasPorFamilia as $idFam => $grupo) {
        $items = isset($grupo['items']) && is_array($grupo['items']) ? array_values($grupo['items']) : [];
        if ($items !== []) {
            $queues[(int) $idFam] = $items;
        }
    }
    ...
    while ($queues !== []) {
        $added = false;
        foreach (array_keys($queues) as $idFam) {
            if (!isset($queues[$idFam]) || $queues[$idFam] === []) {
                unset($queues[$idFam]);
                continue;
            }
            $out[] = array_shift($queues[$idFam]);
            $added = true;
            if ($queues[$idFam] === []) {
                unset($queues[$idFam]);
            }
...
foreach ($piezasPorFamilia as $idFamDest => $grupoDest) {
    ...
    foreach (($grupoDest['items'] ?? []) as $cand) {
        ...
        $imgCand = joyeria_resolver_url_imagen((string) ($cand['url_imagen'] ?? ''));
        if ($imgCand === null) {
            continue;
        }
        $piezaRef = $cand;
...
$promocionalesCatalogo = array_merge(
    ...
    joyeria_cargar_promociones_banner_catalogo('visitante')
);
...
<body<?php echo $promoBarInferiorActiva ? ' class="has-promo-bar-inferior"' : ''; ?>>
    ...
    <header class="header">
        ...
        <div class="header-icons">
            ...
            <div class="tienda-header-wrap d-flex align-items-center gap-2">
                <?php if ($isCliente): ?>
                    <a href="user/cuenta.php" class="icon btn-account border-0 bg-transparent p-0" title="Mi cuenta" aria-label="Mi cuenta">
                        <svg viewBox="0 0 24 24">
                ...
                <?php else: ?>
                    <a href="login.php" class="icon btn-account border-0 bg-transparent p-0" id="btnTiendaAccount" title="Tu cuenta" aria-label="Iniciar sesión o registrarse">
                        <svg viewBox="0 0 24 24">
                            <circle cx="12" cy="8" r="4"></circle>
                            <path d="M4 20c0-4 4-6 8-6s8 2 8 6"></path>
                        </svg>
        ...
        <div class="vitrina-inner">
            <div class="vitrina-stage">
                <?php foreach ($vitrinaNarrativas as $vi => $n):
                    ...
                    $imgUrlV = $piezaV ? joyeria_resolver_url_imagen((string) ($piezaV['url_imagen'] ?? '')) : null;
                ...
                <div class="catalog-carousel-track familias-spotlight-track" tabindex="0" role="region" aria-label="Carrusel de familias destacadas">
                    <?php foreach ($familiasDestacadas as $fd): ?>
                        <?php
                            ...
                            $imgFam = htmlspecialchars((string) ($fd['img'] ?? ''), ENT_QUOTES, 'UTF-8');
                        ?>
