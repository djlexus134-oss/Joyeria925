...
if [[ -z "$DOMAIN" ]]; then
  echo "Define JOYERIA_DOMAIN en $ENV_FILE o deja sites-available/joyeria con server_name."
  exit 1
fi
...
mkdir -p /var/log/joyeria
chown www-data:www-data /var/log/joyeria 2>/dev/null || chown root:www-data /var/log/joyeria
chmod 750 /var/log/joyeria
